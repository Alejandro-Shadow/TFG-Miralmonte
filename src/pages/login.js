...
export function renderLogin() {
      ...
      <div style="width:100%;max-width:400px;padding:var(--space-4)">
        ...
        <div class="card" style="padding:32px">
          <div id="login-form">
            ...
            <button id="login-btn" class="btn btn-primary" style="width:100%;margin-top:20px">Iniciar Sesión</button>
            <p style="text-align:center;margin-top:16px;font-size:var(--text-sm);color:var(--text-muted)">
              ¿No tienes cuenta? <a href="#" id="register-link" style="color:var(--primary-400);text-decoration:none;font-weight:600">Crear cuenta</a>
            </p>
          </div>
          <div id="register-form" style="display:none">
            ...
            <button id="register-btn" class="btn btn-primary" style="width:100%;margin-top:20px">Crear Cuenta</button>
            <p style="text-align:center;margin-top:16px;font-size:var(--text-sm);color:var(--text-muted)">
              ¿Ya tienes cuenta? <a href="#" id="login-link" style="color:var(--primary-400);text-decoration:none;font-weight:600">Iniciar sesión</a>
            </p>
          </div>
        </div>
      </div>
  ...
  document.getElementById('login-btn')?.addEventListener('click', async () => {
    ...
    if (result.success) { window.location.reload(); } else {
      showToast('Error: ' + result.error, 'error'); btn.disabled = false; btn.textContent = 'Iniciar Sesión';
    }
  });
  ...
  document.getElementById('register-btn')?.addEventListener('click', async () => {
    ...
    if (result.success) { showToast('Cuenta creada. Revisa tu email para confirmar.', 'success'); }
